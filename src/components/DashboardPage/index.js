...
//   const { query = '', results, page, totalPages, contentType, sortBy, timeFrame } = useSelector((state) => state.dashboard); // Default to empty string
...
function DashboardPage() {
  ...
  const handleWelcomeClick = () => {
    dispatch(setQuery('')); // Clear the query
    dispatch(setPage(0)); // Reset page to 0
    navigate('/dashboard'); // Navigate without query parameters
  };
...
//     // Only include the query in the URL if it's not empty
//     const searchQuery = query ? `?query=${encodeURIComponent(query)}&page=${newPage}` : `?page=${newPage}`;
